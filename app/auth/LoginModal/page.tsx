...
import { Eye, EyeOff, Lock, Mail, X, Smartphone, KeyRound, Loader, User } from "lucide-react";
  ...
  const handleEmailLogin = async (data: FormData) => {
    ...
    try {
      const res: any = await axios.post("https://ecomapi.ftdigitalsolutions.org/user_login/", {
        ...
        vendor_id: vendorId,
        ...(data?.referrer_user_code && { referrer_user_code: data.referrer_user_code }),
      });
        ...
        <div className="mb-6 flex justify-center">
          <div className="inline-flex rounded-full bg-gray-100 p-1 shadow-md">
            <button
              ...
              className={`px-4 py-1 rounded-full ${loginType === 'MOBILE' ? 'bg-[#a8822d] text-white' : 'text-gray-700 hover:bg-gray-200'}`}
            >
              Mobile Login
            </button>
            <button
              onClick={() => { setLoginType('EMAIL'); setStep('LOGIN'); setError(''); }}
              className={`px-4 py-1 rounded-full ${loginType === 'EMAIL' ? 'bg-[#a8822d] text-white' : 'text-gray-700 hover:bg-gray-200'}`}
            >
              Email Login
            </button>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          {loginType === 'EMAIL' && (
            <>
              ...
              <div className="text-right">
                <Link href="/auth/forgot-password" className="text-[#a8822d] hover:text-green-500 text-sm">Forgot password?</Link>
              </div>
            </>
          )}
          ...
          {loginType === 'MOBILE' && (
            <>
              ...
              {/* referrels */}
              <div>
                <label htmlFor="referrer_user_code" className="block text-sm font-medium text-gray-700">Referrer code</label>
                <div className="relative mt-1">
                  <User className="absolute left-3 top-1 h-5 w-5 text-gray-400" />
                  <input
                    id="referrer_user_code"
                    type="text"
                    {...register('referrer_user_code')}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full flex justify-center gap-2 py-2 px-4 rounded-md text-white bg-[#a8822d] hover:bg-green-700"
          >
            {loginType === 'EMAIL'
              ? 'Login'
              : step === 'LOGIN'
          ...
          <p className="text-sm text-center text-gray-600">
            ...
            <Link href="/auth/register" className="font-medium text-[#a8822d] hover:text-green-500">
              Create one
            </Link>
          </p>
        </form>
