...
export default function CreateAccountPage() {
  ...
  const [referrerUserCode, setReferrerUserCode] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      , ...(referrerUserCode && { referrer_user_code: referrerUserCode })
    }
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* referrels */}
            <div>
              <label htmlFor="referrer_user_code" className="block text-sm font-medium text-gray-700">
                Referrer User Code
              </label>
              <div className="mt-1 relative">
                <User className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-5 w-5" />
                <input
                  ...
                  type="text"
                  value={referrerUserCode}
                  onChange={(e) => setReferrerUserCode(e.target.value)}
                  className="pl-10 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full flex gap-2 justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#a8822d] hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
            >
            ...
            <p className="text-sm text-gray-600 text-center">
              ...
              <Link href="/auth/login" className="font-medium text-[#a8822d] hover:text-green-600">
                Sign in
              </Link>
            </p>
          </form>
